...
+(void)setQRRequest:(NSString *)url image:(void (^)(UIImage *imageQr))img
{
//    [[[ThirdpartyImpl alloc]init] t_requestSinaShortUrl:[ThirdPartyBean modelWithDictionary:@{@"source":__UMID_SINA_KEY__,@"url_long":url}] complete:^(ThirdPartyModel *model) {
//        if(model.url_short != nil){
//            //        model.url_short
//           img([SGQRCodeTool setQRurl:model.url_short isError:YES]);
//        }else{
//            img([SGQRCodeTool setQRurl:url isError:YES]);
//        }
...
+(UIImage *)setQRurl:(NSString *)url isError:(BOOL)isError
{
    if (isError == YES) {
        return  [SGQRCodeTool SG_generateWithLogoQRCodeData:[NSString decodeFromPercentEscapeString:url]?:@"" logoImageName:@"about_bird_1" logoScaleToSuperView:0.2];
    }else{
        NSString * strUrl = url;
        
//        if ([Helper justWithInitRegularly:[HistoryData Scoredomain] Str:strUrl]) {
//            NSArray *temp=[url componentsSeparatedByString:@"?"];
//            if (temp.count != 0) {
//                strUrl = temp[0];
//            }
//        }
        return [SGQRCodeTool SG_generateWithLogoQRCodeData:[NSString decodeFromPercentEscapeString:strUrl]?:@"" logoImageName:@"about_bird_1" logoScaleToSuperView:0.2];
    }
