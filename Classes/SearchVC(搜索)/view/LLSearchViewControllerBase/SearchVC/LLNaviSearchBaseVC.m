...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [STCModel stcModel].vcType = 1;
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(clipbosrdPop:) name:CLIPBOARDREQUESTPOP object:nil];
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(clipbosrdDismiss:) name:CLIPBOARDREQUESTPOPDismiss object:nil];
...
//    [[NSNotificationCenter defaultCenter] removeObserver:self name:CLIPBOARDREQUESTPOP object:nil];
//    [[NSNotificationCenter defaultCenter] removeObserver:self name:CLIPBOARDREQUESTPOPDismiss object:nil];
...
#pragma mark - 通知处理
//-(void)clipbosrdPop:(NSNotification *)notification{
//    [self.searchBar resignFirstResponder];
//
//}
//-(void)clipbosrdDismiss:(NSNotification *)notification{
////    [self.searchBar becomeFirstResponder];
//
//}
...
//    self.searchBar.text =  iModel.Title;
//    //通知外界开始搜索
//    [self beignToSearch:NaviBarSearchTypeDefault cellP:nil tagLabel:nil searchBar:_searchBar];
//    //保存搜索历史记录
//    [self.shopHistoryP saveSearchCache:_searchBar.text result:nil];
//}
...
- (void)setupSearchNaviBar
{
    ...
    searchNaviBarView.searbarPlaceHolder = @"输入淘宝/天猫 商品标题或关键词";
    searchNaviBarView.isHideSearchBarRightIcon = NO;
    searchNaviBarView.searchBarRightIconImage = [UIImage imageNamed:@"discount_search_icon"];
...
- (UIScrollView *)myBGScrollView
{
    if (!_myBGScrollView) {
        _myBGScrollView = ({
            UIScrollView *bgScrollView =  [[UIScrollView alloc] initWithFrame:CGRectMake(0,ZYHT_StatusBarAndNavigationBarHeight, ZYHT_ScreenWidth,ZYHT_ScreenHeight-ZYHT_StatusBarAndNavigationBarHeight)];
            bgScrollView.backgroundColor = [UIColor colorWithRed:245/255.0 green:245/255.0 blue:245/255.0 alpha:1.0];
            bgScrollView.contentSize = CGSizeMake(ZYHT_ScreenWidth, ZYHT_ScreenHeight-ZYHT_StatusBarAndNavigationBarHeight-4);
