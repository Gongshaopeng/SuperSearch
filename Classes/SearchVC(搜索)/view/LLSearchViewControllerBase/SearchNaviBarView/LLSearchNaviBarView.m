...
- (instancetype)init{
    if (self = [super init]) {
        ...
        leftRightIconWH = __kNewSize(88);
        searchIconWH = __kNewSize(28);
        LeadMargin = __kNewSize(13*2);
        TrailMargin = __kNewSize(26);
        searchBgViewH = __kNewSize(58);
...
- (UIView *)setupSearView
{
    ...
    self.searLogoIconV = ({
        UIImageView *searLogoIconV = [[UIImageView alloc] initWithFrame:(CGRect){0, 0, searchIconWH, searchIconWH}];
    ...
    self.searchBar = ({
        LLSearchBar *searchBar = [[LLSearchBar alloc] initWithFrame:CGRectMake(0, 0, self.searchBgView.bounds.size.width-10, self.searchBgView.bounds.size.height)];
...
- (void)modifySearchBarViewFrame
{
    ...
    if (self.searLogoIconV.hidden == NO) {
        self.searLogoIconV.x = searchIconWH;
        self.searchBar.x =  searchIconWH +self.searLogoIconV.width;
        self.searchBar.width = self.searchBgView.bounds.size.width ;
