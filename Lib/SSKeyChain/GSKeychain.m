...
NSString *const kGSKeychainErrorDomain = @"com.samsoffes.GSKeychain";

NSString *const kGSKeychainAccountKey = @"acct";
NSString *const kGSKeychainCreatedAtKey = @"cdat";
NSString *const kGSKeychainClassKey = @"labl";
NSString *const kGSKeychainDescriptionKey = @"desc";
NSString *const kGSKeychainLabelKey = @"labl";
NSString *const kGSKeychainLastModifiedKey = @"mdat";
NSString *const kGSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef GSKeychainAccessibilityType = NULL;
#endif

@interface GSKeychain ()
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = GSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kGSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (GSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return GSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (GSKeychainAccessibilityType) {
		CFRelease(GSKeychainAccessibilityType);
	}
	GSKeychainAccessibilityType = accessibilityType;
}
#endif
